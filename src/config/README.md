...
| `commentConfig.ts` | 评论系统配置（Halo、Twikoo、Waline、Artalk、Giscus、Disqus） |
...
| `haloConfig.ts` | Halo 内容 API 地址（默认 `http://127.0.0.1:8090`） |
...
- 所有配置文件均可通过 `index.ts` 统一导入。部署到 Halo + Firefly 时，站点、外观、壁纸、导航、侧栏、公告、资料和音乐会优先读取 Halo 控制台的 `Firefly 前端` 设置，源码配置作为默认值兜底。
